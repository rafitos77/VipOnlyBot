...
# Ensure project root is on sys.path (python app/flow_stress_test.py)
PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
if PROJECT_ROOT not in sys.path:
    sys.path.insert(0, PROJECT_ROOT)

# Importar módulos (versão atual do projeto)
from app.config import Config
from app.users_db import UserDB
from app.payments import PaymentCreateResult, payment_provider_for
...
class MockPaymentGateway:
    """Simula os providers novos sem chamar APIs externas."""
    def create(self, *, user_id: int, plan: str, lang: str, amount: float, currency: str) -> PaymentCreateResult:
        ...
        return PaymentCreateResult(
            provider="pushinpay",
            external_id="pp_test_123",
            amount=amount,
            currency="BRL",
            pix_qr_code="0002012658...PIX-CODE-TEST...",
        )
    ...
    async def simulate_callback_query(self, user_id, data):
        if data.startswith("sel:"):
            ...
            if payment.provider == "stripe":
                logger.info(
                    f"USER {user_id}: Checkout Stripe criado para {plan} ({price} {pricing['currency']}). URL: {payment.checkout_url}"
                )
            else:
                logger.info(
                    f"USER {user_id}: PIX PushinPay criado para {plan} ({price} BRL). ID: {payment.external_id}"
                )
