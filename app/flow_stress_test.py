    ...
    async def simulate_callback_query(self, user_id, data):
        if data.startswith("sel:"):
            ...
            if pricing['currency'] == 'BRL':
                pix_charge = self.pushinpay_client.create_pix_charge(price, f"test_ref_{user_id}", "https://example.com/webhook")
                logger.info(f"USER {user_id}: Charge Pix criado para {plan} ({price} {pricing['currency']}). ID: {pix_charge['id']}")
            else:
                logger.info(f"USER {user_id}: Stripe payment would be created for {plan} ({price} {pricing['currency']})")
