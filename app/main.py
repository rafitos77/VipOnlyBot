        ...
        class VIPBotUltra:
            ...
            async def cmd_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
                        ...
                        if referrer_id != user.id:
                            ...
                            try:
                                ...
                                await self.app.bot.send_message(
                                    chat_id=referrer_id,
                                    text=escape_html(get_text("referral_reward_msg", ref_lang)),
                ...
                if not u_data.get('language') or (context.args and 'lang' in context.args):
                    ...
                    await msg.reply_text(
                        escape_html(get_text("select_lang")), 
                    ...
                    try:
                        ...
                        await status_msg.edit_text(
                            f"{escape_html(welcome_title)}\n\n{escape_html(welcome_copy)}",
                            reply_markup=keyboard,
                            parse_mode=ParseMode.HTML
                        )
                    except:
            ...
            async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
                ...
                if text == get_text("btn_search", lang):
                    await update.message.reply_text(escape_html(get_text("search_prompt", lang)), parse_mode=ParseMode.HTML)
                    ...
                    await update.message.reply_text(
                        escape_html(get_text("referral_copy", lang, link=ref_link, count=u_data.get('referral_count', 0), credits=u_data.get('credits', 0))),
                        parse_mode=ParseMode.HTML
                    )
                
                elif text == get_text("btn_stats", lang):
                    is_vip = "✅ VIP" if await user_db.async_is_license_active(user_id) else "❌ FREE"
                    stats_html = f"👤 <b>{escape_html(update.effective_user.first_name)}</b>\n\n"
                    stats_html += f"Status: {escape_html(is_vip)}\n"
                    stats_html += f"Créditos: {escape_html(str(u_data.get('credits', 0)))}\n"
                    stats_html += f"Convidados: {escape_html(str(u_data.get('referral_count', 0)))}"
                    ...
                    await update.message.reply_text(escape_html(get_text("welcome_copy", lang)), parse_mode=ParseMode.HTML)
                    ...
                    await update.message.reply_text(
                        escape_html(get_text("god_mode_msg", lang, mode=mode_text)),
                        reply_markup=keyboard,
                        parse_mode=ParseMode.HTML
                    )
            ...
            async def show_payment_popup(self, update: Update, user_id: int, lang: str, is_downsell: bool = False):
                ...
                msg = update.effective_message
                await msg.reply_text(escape_html(f"{title}\n\n{copy}"), reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=ParseMode.HTML)
            ...
            async def on_callback_query(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
                ...
                if data.startswith("setlang:"):
                    ...
                    await query.edit_message_text(f"🔄 {escape_html(get_text('loading', lang))} <b>{escape_html(name)}</b>", parse_mode=ParseMode.HTML)
                    
                    async with MediaFetcher() as fetcher:
                        creator = {'service': service, 'id': c_id, 'name': name}
                        items = await fetcher.fetch_posts_paged(creator, offset=0)
                        if not items:
                            await query.edit_message_text(escape_html(get_text("nothing_found", lang)), parse_mode=ParseMode.HTML)
                            return
                        ...
                        if has_access:
                            if used_credit:
                                await query.message.reply_text(escape_html(get_text("using_credit", lang)), parse_mode=ParseMode.HTML)
                            
                            text = escape_html(get_text("model_found", lang, name=name))
                            kb = [[InlineKeyboardButton(get_text("btn_download_all", lang), callback_data=f"dlall:{service}:{c_id}:{name[:20]}")],
                                  [InlineKeyboardButton(get_text("btn_view_page", lang), callback_data=f"page:{service}:{c_id}:0:{name[:20]}")]]
                            ...
                            await query.edit_message_text(escape_html(get_text("sending_previews", lang, name=name)), parse_mode=ParseMode.HTML)
                            for item in items[:3]:
                                if await fetcher.download_media(item):
                                    await self.uploader.upload_and_cleanup(item, user_id, caption=escape_html(f"🔥 Preview: {name}"))
                    ...
                    await query.edit_message_text(escape_html(get_text("downloading", lang, name=name)), parse_mode=ParseMode.HTML)
                    async with MediaFetcher() as fetcher:
                        creator = {'service': service, 'id': c_id, 'name': name}
                        ...
                        for i in range(0, min(len(items), 50), 10):
                            ...
                            for item in batch:
                                if await fetcher.download_media(item):
                                    await self.uploader.upload_and_cleanup(item, user_id, caption=escape_html(f"✅ {name} - VIP"))
                                    await asyncio.sleep(1.2)
                            await asyncio.sleep(2)
                        await query.message.reply_text(escape_html(get_text("download_complete", lang)), parse_mode=ParseMode.HTML)
                elif data.startswith("stop:"):
                    # Stop pagination / navigation - clear keyboard and notify user
                    parts = data.split(":", 4)
                    if len(parts) >= 5:
                        _, service, c_id, page_str, name = parts
                    else:
                        # Fallback generic stop
                        await query.edit_message_text(escape_html(get_text("operation_stopped", lang) if 'operation_stopped' in globals() else "Operation stopped."), parse_mode=ParseMode.HTML)
                        return

                    await query.edit_message_text(escape_html(f"Pagination stopped for {name[:40]}."), parse_mode=ParseMode.HTML)
                    logger.info(f"User {user_id} stopped pagination for model={name}")
                elif data.startswith("page:"):
                    # Pagination for VIP - shows a single page of PAGE_SIZE items
                    parts = data.split(":", 4)
                    if len(parts) < 5:
                        await query.answer(escape_html(get_text("error_occurred", lang, error="Invalid page data")))
                        return
                    _, service, c_id, page_str, name = parts
                    try:
                        page = int(page_str)
                    ...
                    await query.edit_message_text(escape_html(f"{get_text('loading', lang)} - {name} (Page {page + 1})"), parse_mode=ParseMode.HTML)
                    async with MediaFetcher() as fetcher:
                        creator = {'service': service, 'id': c_id, 'name': name}
                        items = await fetcher.fetch_posts_paged(creator, offset=offset)
                        total = len(items)
                        batch = items[:PAGE_SIZE]
                        sent = 0
                        logger.info(f"Pagination: model={name}, page={page}, offset={offset}, total_found={total}")
                        for item in batch:
                            if await fetcher.download_media(item):
                                if await self.uploader.upload_and_cleanup(item, user_id, caption=escape_html(f"✅ {name} - VIP")):
                                    sent += 1
                                await asyncio.sleep(1.2)
                        ...
                        nav_buttons = []
                        if page > 0:
                            nav_buttons.append(InlineKeyboardButton("⬅️ Prev", callback_data=f"page:{service}:{c_id}:{page-1}:{name[:20]}"))
                        if total > PAGE_SIZE:
                            nav_buttons.append(InlineKeyboardButton("➡️ Next", callback_data=f"page:{service}:{c_id}:{page+1}:{name[:20]}"))
                        nav_kb = [nav_buttons] if nav_buttons else []
                        # Add Stop button (clears navigation) and Download All button
                        control_row = [
                            InlineKeyboardButton("⏹️ Stop", callback_data=f"stop:{service}:{c_id}:{page}:{name[:20]}"),
                            InlineKeyboardButton(get_text("btn_download_all", lang), callback_data=f"dlall:{service}:{c_id}:{name[:20]}")
                        ]
                        nav_kb.append(control_row)

                        await query.edit_message_text(escape_html(f"Enviado(s): {sent} item(s) — Página {page + 1}"), reply_markup=InlineKeyboardMarkup(nav_kb), parse_mode=ParseMode.HTML)
                        logger.info(f"Pagination sent items={sent} for model={name} page={page}")
                    ...
                    if pricing['currency'] == 'USD':
                        ...
                        if pix_charge and pix_charge.get("status") == "pending":
                            ...
                            await query.edit_message_text(
                                escape_html(get_text("pix_order_created", lang, label=pricing['label'])),
                            ...
                            if qr_code_image:
                                await context.bot.send_photo(
                                    chat_id=user_id,
                                    photo=qr_code_image,
                                    caption=escape_html(f"{qr_code_text}\n\n{get_text('pix_scan_qr', lang)}"),
                                    parse_mode=ParseMode.HTML
                                )
                            else:
                                # Fallback: send the QR code text only
                                await context.bot.send_message(chat_id=user_id, text=escape_html(f"{qr_code_text}\n\n{get_text('pix_scan_qr', lang)}"), parse_mode=ParseMode.HTML)
                        else:
            ...
            async def successful_payment_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
                ...
                if plan_type:
                    ...
                    await message.reply_text(
                        escape_html(get_text("payment_success", lang)),
                        parse_mode=ParseMode.HTML
                    )
                    logger.info(f"License {plan_type} activated for user {user_id} via Stripe")
                else:
                    await message.reply_text(
                        escape_html(get_text("payment_success_no_plan", lang)),
                        parse_mode=ParseMode.HTML
                    )
