    ...
    try:
        ...
        class VIPBotUltra:
            ...
            async def on_callback_query(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
                ...
                if data.startswith("setlang:"):
                    ...
                    if pricing['currency'] == 'USD':
                        ...
                        # Include plan_type in payload for recovery if context is lost
                        payload = f"VIP_{plan}_{user_id}"
                        ...
                        pix_charge = pushinpay_client.create_pix_charge(price, reference_id, webhook_url)
                        if pix_charge and pix_charge.get("status") == "pending":
                            qr_code_image = pix_charge["qr_code_image"]
                            qr_code_text = pix_charge["qr_code_text"]
                            
                    # Store pending transaction in DB (idempotent - won't duplicate)
                    user_db.add_transaction(
                        user_id=user_id,
                        transaction_id=pix_charge["id"],
                        ...
                        status_detail=None
                            ...
                            await query.edit_message_text(
                                ...
                                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(get_text("btn_pix_confirm", lang), callback_data=f"check_pix:{pix_charge['id']}")]]),
                                parse_mode=ParseMode.MARKDOWN
                            )
                    ...
                    with user_db._get_conn() as conn:
                        ...
                        if trans:
                            ...
                            if trans_dict['status'] == 'PAID':
                                # Payment confirmed, activate license (idempotent)
                                plan_type = trans_dict['plan_type']
                                # Check if already activated to avoid duplicate messages
                                if user_db.is_license_active(user_id):
                                    await query.edit_message_text(get_text("payment_success", lang))
                                else:
                                    user_db.activate_license(user_id, plan_type)
                                    await query.edit_message_text(get_text("payment_success", lang))
                            elif trans_dict['status'] == 'PENDING':
                                # Still pending, check with Pushin Pay API
                                try:
                                    charge_data = pushinpay_client.get_charge_status(transaction_id)
                                    
                                    if charge_data:
                                        if charge_data.get('status') == 'paid':
                                            user_db.update_transaction_status(transaction_id, 'PAID', 'Payment confirmed via API')
                                            plan_type = trans_dict['plan_type']
                                            # Check if already activated to avoid duplicate messages
                                            if user_db.is_license_active(user_id):
                                                await query.edit_message_text(get_text("payment_success", lang))
            ...
            async def successful_payment_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
                ...
                if plan_type:
                    # Store transaction (idempotent - won't duplicate)
                    user_db.add_transaction(
                        user_id=user_id,
                    ...
                    # Check if already active to avoid duplicate success messages
