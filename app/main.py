...
async def run_bot():
    ...
    try:
        ...
        from app.payments import create_payment_for_user, payment_provider_for
        from app.payments import PushinPayClient
        ...
        class VIPBotUltra:
            ...
            def _esc_md(self, s: str) -> str:
                ...
                if s is None:
                    return ""
                # Minimal escapes for ParseMode.MARKDOWN
                for ch in ["_", "*", "[", "]", "(", ")", "`"]:
                    s = s.replace(ch, f"\{ch}")
                return s
            ...
            async def on_callback_query(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
                ...
                if data.startswith("setlang:"):
                    ...
                    await self.safe_edit(query, f"🔄 {get_text('loading', lang)} **{self._esc_md(name)}**...", parse_mode=ParseMode.MARKDOWN)
                    
                    async with MediaFetcher() as fetcher:
                        creator = {'service': service, 'id': c_id, 'name': name}
                        items = await fetcher.fetch_posts_paged(creator, offset=0)
                        if not items:
                            await self.safe_edit(query, get_text("nothing_found", lang))
                            return
                        ...
                        if has_access:
                            ...
                            kb = [[InlineKeyboardButton(get_text("btn_download_all", lang), callback_data=f"dlall:{service}:{c_id}:{name[:20]}")],
                                  [InlineKeyboardButton(get_text("btn_view_page", lang), callback_data=f"dlpage:{service}:{c_id}:{name[:20]}:0")]]
                            await self.safe_edit(query, text, reply_markup=InlineKeyboardMarkup(kb), parse_mode=ParseMode.MARKDOWN)
                        else:
                            if not user_db.check_preview_limit(user_id):
                                await self.show_payment_popup(update, user_id, lang)
                                return
                            user_db.increment_previews(user_id)
                            await self.safe_edit(query, get_text("sending_previews", lang, name=name))
                            for item in items[:3]:
                    ...
                    # dlall = start first page
                    # dlpage = view specific offset (legacy)
                    # dlnext = next page
                    # dlstop = stop session
                
                    if data.startswith("dlstop:"):
                        try:
                            _, service, c_id, name = data.split(":", 3)
                        except Exception:
                            service = c_id = name = ""
                        try:
                        ...
                        await self.safe_edit(query, "⛔ Download interrompido.", parse_mode=ParseMode.MARKDOWN)
                        return
                    ...
                    c_id = parts[2]
                    name = parts[3]
                    offset = int(parts[4]) if (action in ("dlpage", "dlnext") and len(parts) > 4) else 0

                    # Session continuation: for dlnext, prefer stored offset (prevents wrong offsets and ensures continuity)
                    ...
                    if action == "dlall":
                        self._dl_sessions[user_id]["offset"] = 0
                        offset = 0

                
                    await self.safe_edit(query, get_text("downloading", lang, name=self._esc_md(name)), parse_mode=ParseMode.MARKDOWN)
                
                    PAGE_SEND_LIMIT = 50  # avoid flooding
                    async with MediaFetcher() as fetcher:
                        creator = {"service": service, "id": c_id, "name": name}
                        items = await fetcher.fetch_posts_paged(creator, offset=offset)
                
                        if not items:
                            await self.safe_edit(query, f"✅ Download completo: **{self._esc_md(name)}**\n\nNão há mais páginas.", parse_mode=ParseMode.MARKDOWN)
                            return
                
                        items = items[:PAGE_SEND_LIMIT]
                        sent_any = False
                        ...
                        next_offset = offset + max(1, len({it.post_id for it in items if getattr(it, "post_id", None)}))
                        ...
                        kb = [
                            [InlineKeyboardButton("▶️ Baixar próxima página", callback_data=f"dlnext:{service}:{c_id}:{name}:{next_offset}")],
                            [InlineKeyboardButton("⛔ Parar", callback_data=f"dlstop:{service}:{c_id}:{name}")]
                        ]
                    ...
                    try:
                        ...
                        tx = client.get_transaction(external_id)
                        status = (tx.get("status") or "").lower()

                        if status == "paid":
                            # map payment -> user/plan
                            rec = user_db.get_payment_by_external_id(provider, external_id)
                            if rec:
                                user_db.mark_payment_paid(provider, external_id, raw_payload=str(tx))
                    ...
                    # Public base URL for webhooks / redirect pages (recommended on Railway)
                    ...
                    try:
                        ...
                        if payment.provider == "stripe":
                            ...
                            # PushinPay PIX: send QR image (if available) + copy/paste code + a "check" button.
                            if payment.pix_qr_code_png_bytes:
                                try:
                                    await query.message.reply_photo(payment.pix_qr_code_png_bytes)
                                except Exception:
                                    pass

                            pix_code = payment.pix_qr_code or ""
                            msg = get_text("payment_created_pix", lang) + "\n\n" + f"```\n{pix_code}\n```"
                            kb = InlineKeyboardMarkup(
                                [[InlineKeyboardButton(get_text("btn_check_payment", lang), callback_data=f"checkpay:pushinpay:{payment.external_id}")]]
                            )
                            await self.safe_edit(query, msg, reply_markup=kb, parse_mode=ParseMode.MARKDOWN)
        ...
        async def start_webhook_server(tg_bot):
            ...
            async def pushinpay_webhook(request):
                ...
                token_expected = os.getenv("PUSHINPAY_WEBHOOK_TOKEN")
                if token_expected:
                ...
                if status == "paid":
                    if user_db.is_payment_already_paid("pushinpay", external_id):
                        return web.Response(text="Already processed")

                    rec = user_db.get_payment_by_external_id("pushinpay", external_id)
                    if rec:
                        try:
                            user_id_ = int(rec["user_id"])
                            plan_type = rec["plan_type"]
                            user_db.mark_payment_paid("pushinpay", external_id, raw_payload=str(data))
                            user_db.activate_license(user_id_, plan_type)
                            lang_ = user_db.get_user(user_id_).get("language", "pt")
                            await tg_bot.send_message(chat_id=user_id_, text=get_text("payment_confirmed", lang_))
                        except Exception as e:
                            logger.exception(f"PushinPay webhook processing error: {e}")
                            return web.Response(status=500, text="Processing error")
