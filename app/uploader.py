...
class TelegramUploader:
    ...
    async def _upload_batch(self, channel_id: int, media_items: List[MediaItem],
        ...
        try:
            ...
            # NOTE: In python-telegram-bot, InputMedia objects are immutable-ish; setting attributes may error.
            # Recreate the first element with caption instead.
            if media_group and caption:
                first = media_group[0]
                try:
                    if isinstance(first, InputMediaVideo):
                        media_group[0] = InputMediaVideo(first.media, caption=caption, parse_mode=ParseMode.MARKDOWN)
                    else:
                        media_group[0] = InputMediaPhoto(first.media, caption=caption, parse_mode=ParseMode.MARKDOWN)
                except Exception:
