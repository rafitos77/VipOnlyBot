...
def asaas_available() -> bool:
    return bool(os.getenv("ASAAS_ACCESS_TOKEN"))


def nowpayments_available() -> bool:
    return bool(os.getenv("NOWPAYMENTS_API_KEY") and os.getenv("NOWPAYMENTS_IPN_SECRET"))
...
def create_payment_explicit(
    *,
    ...
    if provider in ("asaas", "pix"):
        client = AsaasClient()
        cust_id = client.create_customer(
            user_id=user_id,
            name=f"Telegram User {user_id}",
            email=f"tg{user_id}@example.com",
        )
        return client.create_pix_charge(
            user_id=user_id,
            plan=plan,
            amount=amount,
            currency=currency,
            customer_id=cust_id,
            base_url=base_url,
            description=f"VIP {plan} (Telegram {user_id})",
        )
...
def create_payment_for_user(
    *,
    ...
    if provider == "asaas":
        client = AsaasClient()
        # Create a lightweight customer for this Telegram user
        cust_id = client.create_customer(
            user_id=user_id,
            name=f"Telegram User {user_id}",
            email=f"tg{user_id}@example.com",
