...
class UserDB:
    ...
    def __init__(self, db_path: str = None):
        if db_path is None:
            # Respect explicit DB_PATH (Railway volume should mount at /data).
        ...
        with self._get_conn() as conn:
            ...
            try:
                ...
                if "asaas_customer_id" not in cols:
                    cursor.execute("ALTER TABLE users ADD COLUMN asaas_customer_id TEXT")
            except Exception:
                # Non-fatal: on older sqlite versions / permissions, ignore.
                pass
    ...
    def is_license_active(self, user_id: int) -> bool:
        ...
        # Global admin/test switches
        # ADMIN_FORCE_VIP=1 => force VIP for everyone (useful for smoke tests / staging)
        if str(os.getenv('ADMIN_FORCE_VIP', '0')).lower() in ('1', 'true', 'yes', 'on'):
            return True

        # God mode is a DB toggle (⚡ MODO GOD). It can be used for admin testing, but
        # it's evaluated for any user row to keep the logic simple and consistent.
