...
class UserDB:
    ...
    def _init_db(self):
        ...
        with self._get_conn() as conn:
            ...
            ''')
            # Create transactions table if missing (includes transaction_id column for new DBs)
            cursor.execute('''
            ...
            # Ensure transaction_id column exists in existing older databases.
            # Use PRAGMA table_info to inspect columns and ALTER TABLE ADD COLUMN if missing.
            try:
                cursor.execute("PRAGMA table_info(transactions)")
                cols = [row[1] for row in cursor.fetchall()]  # row format: (cid,name,type,notnull,dflt_value,pk)
                if 'transaction_id' not in cols:
                    # Add the missing column without destroying existing data
                    cursor.execute("ALTER TABLE transactions ADD COLUMN transaction_id TEXT")
            except Exception as e:
                # If inspection or alteration fails, log and continue; index creation below will handle absence.
                logger.warning(f"Could not ensure transaction_id column exists: {e}")

            # Now create the unique index on transaction_id (idempotent). This will succeed only if column exists.
            try:
                cursor.execute('''
                    CREATE UNIQUE INDEX IF NOT EXISTS idx_transactions_txid ON transactions(transaction_id)
                ''')
            except Exception as e:
                logger.warning(f"Could not create unique index idx_transactions_txid: {e}")
    ...
    def add_transaction(self, user_id: int, transaction_id: str, gateway: str, amount: float, currency: str, plan_type: str, status: str, status_detail: Optional[str] = None):
        ...
        with self._get_conn() as conn:
            ...
            try:
                ...
                # If unique constraint violated, a concurrent worker already inserted it.
                logger.warning(f"Transaction {transaction_id} insertion conflict (likely duplicate); skipping.")
                return
        logger.info(f"Transaction {transaction_id} recorded for user {user_id} via {gateway}")

    def update_transaction_id(self, old_transaction_id: str, new_transaction_id: str) -> bool:
        """Update a transaction's transaction_id (e.g., replace temporary reference_id with provider id).
        Returns True if update succeeded, False otherwise.
        """
        with self._get_conn() as conn:
            cursor = conn.cursor()
            try:
                cursor.execute("UPDATE transactions SET transaction_id = ? WHERE transaction_id = ?", (new_transaction_id, old_transaction_id))
                conn.commit()
                if cursor.rowcount == 0:
                    logger.warning(f"No transaction found with id {old_transaction_id} to update")
                    return False
                return True
            except sqlite3.IntegrityError:
                # New transaction_id already exists -> another process updated it concurrently
                logger.warning(f"Failed to update transaction id from {old_transaction_id} to {new_transaction_id} due to uniqueness conflict")
                return False
    ...
    async def async_get_user(self, user_id: int) -> Dict[str, Any]:
        """Async wrapper for get_user to avoid blocking event loop."""
        import asyncio
        return await asyncio.to_thread(self.get_user, user_id)

    async def async_update_user(self, user_id: int, **kwargs):
        import asyncio
        return await asyncio.to_thread(self.update_user, user_id, **kwargs)

    async def async_check_preview_limit(self, user_id: int) -> bool:
        import asyncio
        return await asyncio.to_thread(self.check_preview_limit, user_id)

    async def async_increment_previews(self, user_id: int):
        import asyncio
        return await asyncio.to_thread(self.increment_previews, user_id)

    async def async_use_credit(self, user_id: int) -> bool:
        import asyncio
        return await asyncio.to_thread(self.use_credit, user_id)

    async def async_process_referral(self, new_user_id: int, referrer_id: int):
        import asyncio
        return await asyncio.to_thread(self.process_referral, new_user_id, referrer_id)

    async def async_activate_license(self, user_id: int, plan_type: str):
        import asyncio
        return await asyncio.to_thread(self.activate_license, user_id, plan_type)

    async def async_add_transaction(self, user_id: int, transaction_id: str, gateway: str, amount: float, currency: str, plan_type: str, status: str, status_detail: Optional[str] = None):
        import asyncio
        return await asyncio.to_thread(self.add_transaction, user_id, transaction_id, gateway, amount, currency, plan_type, status, status_detail)

    async def async_update_transaction_status(self, transaction_id: str, status: str, status_detail: Optional[str] = None):
        import asyncio
        return await asyncio.to_thread(self.update_transaction_status, transaction_id, status, status_detail)

    async def async_is_license_active(self, user_id: int) -> bool:
        import asyncio
        return await asyncio.to_thread(self.is_license_active, user_id)

    async def async_get_transaction(self, transaction_id: str) -> Optional[Dict[str, Any]]:
        """Return transaction row as dict or None."""
        ...
        def _get(txid):
            with self._get_conn() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute("SELECT * FROM transactions WHERE transaction_id = ?", (txid,))
                row = cursor.fetchone()
                return dict(row) if row else None

        return await asyncio.to_thread(_get, transaction_id)

    async def async_update_transaction_id(self, old_transaction_id: str, new_transaction_id: str) -> bool:
        import asyncio
        return await asyncio.to_thread(self.update_transaction_id, old_transaction_id, new_transaction_id)
